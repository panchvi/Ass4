...
function Get_All_Customers() {
	...
	objRequest.onreadystatechange = function() {
        if (objRequest.readyState == 4 && objRequest.status == 200) {
			...
			for (count = 1; count < (output.GetAllCustomersResult.length-1); count++) {
				customers_table += "<tr><td>" + count + "</td><td>" + output.GetAllCustomersResult[count].CustomerID 
				+ "</td><td>" + output.GetAllCustomersResult[count].CompanyName + "</td><td>" 
				+ output.GetAllCustomersResult[count].City + "</td></tr>"
			}
